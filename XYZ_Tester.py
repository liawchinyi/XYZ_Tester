    ...
    try:
        ...
        if not summary_rows:
            ...
            for row in summary_rows:
                target_r, measured_v_str, knob_settings = row # measured_v_str could be 'ERROR'
                
                # Handle potential 'ERROR' strings in measured_v_str
                if measured_v_str == "ERROR":
                    print(f"  Target Resistance {float(target_r):.2f} Ohms: Measured Voltage: Measurement Failed. Settings: {knob_settings}")
                else:
                    print(f"  Target Resistance {float(target_r):.2f} Ohms: Measured Voltage {float(measured_v_str):.4f} Volts. Settings: {knob_settings}")
